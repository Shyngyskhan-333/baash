...
def detect_collisions():
    ...
    retriever = LegalRetriever()
    problems = asyncio.run(detect_all_problems(retriever))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LexLens RAG / legal NLP pipeline (CPU)")
    parser.add_argument(
        "--build-index",
        nargs="+",
        help="Список ID документов (например: K1500000377 K1400000266) для скачивания и индексации"
    )
    parser.add_argument(
        "--query",
        type=str,
