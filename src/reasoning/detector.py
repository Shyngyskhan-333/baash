...
async def detect_all_problems(retriever: LegalRetriever, top_k_explain: int = 10, doc_ids: Optional[List[str]] = None) -> List[Problem]:
    """
    ...
    Теперь асинхронный для масштабируемого вызова LLM.
    """
    ...
    # ... (outdated detection remains sync/local)
    outdated_anchor = (
    ...
    outdated_candidates = retriever.search_hybrid(outdated_anchor, top_k=10, doc_ids=doc_ids) # reduced top_k for speed
    ...
    for r, c in zip(rows, cols):
        ...
        if doc_ids:
            if anchor_chunk["doc_id"] not in doc_ids and match["doc_id"] not in doc_ids:
                continue
        ...
        if p_id in seen_pairs: continue
        seen_pairs.add(p_id)
    ...
    explained_count = 0
    # Process sequentially for safety, but with await
