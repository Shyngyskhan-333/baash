...
def _get_nli():
    ...
    cache_dir = str(Path("data/models").absolute())
    ...
    try:
        ...
        _model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, cache_dir=cache_dir, local_files_only=True)
    except Exception:

        print(f"[NLI] Модель не найдена локально, скачиваем с HuggingFace...")
        _tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, cache_dir=cache_dir, local_files_only=False)
        _model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, cache_dir=cache_dir, local_files_only=False)
        print(f"[NLI] Модель скачана и сохранена в data/models")

    _model.eval()

    device = "cuda" if torch.cuda.is_available() else "mps" if hasattr(torch.backends, "mps") and torch.backends.mps.is_available() else "cpu"
    print(f"[NLI] Перемещение модели на устройство: {device}")
    _model.to(device)
...
def check_contradictions_batch(pairs: List[Tuple[str, str, str, str]], batch_size: int = 32) -> List[Dict]:
    ...
    for i in tqdm(range(0, len(to_infer), batch_size), desc="NLI Batch Processing"):
        ...
        if (i // batch_size) % 100 == 0 and i > 0:
            ...
            n_done = len(processed_results)
            for pos, global_idx_tmp in enumerate(to_infer_indices[:n_done]):
                id_a_t, id_b_t = pairs[global_idx_tmp][2], pairs[global_idx_tmp][3]
                temp_cache[f"{id_a_t}_{id_b_t}"] = processed_results[pos]
