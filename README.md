...
В корне проекта есть скрипт `setup_fast.ps1`, который автоматически создаст виртуальное окружение, установит все зависимости (Backend и Frontend) и создаст `.env` с правильной кодировкой UTF-8:
...
# Создать .env (Убедитесь, что файл сохранен в UTF-8!)
powershell -Command "Get-Content .env.example -Raw | Out-File -Encoding utf8 .env"
# Отредактируйте .env под вашего провайдера (или оставьте mock)
```

> ⚠️ **Важно**: Если при запуске возникает ошибка `UnicodeDecodeError`, проверьте что файл `.env` сохранен в кодировке **UTF-8** (без BOM). Стандартные команды PowerShell `copy` или `>` могут создавать файлы в UTF-16, что вызывает сбой у `uvicorn`.
