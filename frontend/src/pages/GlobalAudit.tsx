...
const GlobalAudit: React.FC = () => {
  ...
  const startAudit = async () => {
    ...
    setError(null);
    try {
      const data = await runGlobalAudit(activeScope);
      ...
      <div className="max-w-5xl mx-auto space-y-8 px-8 pt-28 pb-16">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 border border-red-500 rounded font-bold transition ${
            auditLoading ? 'bg-red-900/40 text-red-500 cursor-not-allowed' : 'bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white'
          }`}
        >
          {auditLoading ? (
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section>
              <h2 className="text-xl font-bold text-red-400 flex items-center gap-2 border-b border-[#2d3748] pb-2 mb-4">
                <AlertTriangle size={20} /> Противоречия (Коллизии): {auditResults.stats.contradictions}
              </h2>
              <div className="space-y-4">
                {auditResults.contradictions.map((p: any, i: number) => (
            ...
            <section>
              <h2 className="text-xl font-bold text-blue-400 flex items-center gap-2 border-b border-[#2d3748] pb-2 mb-4">
                <FileWarning size={20} /> Дубликаты (Смежные): {auditResults.stats.duplicates}
              </h2>
              <div className="space-y-4">
                {auditResults.duplicates.map((p: any, i: number) => (
            ...
            <section>
              <h2 className="text-xl font-bold text-yellow-400 flex items-center gap-2 border-b border-[#2d3748] pb-2 mb-4">
                <HelpCircle size={20} /> Устаревшие нормы: {auditResults.stats.outdated}
              </h2>
              <div className="space-y-4">
                {auditResults.outdated.map((p: any, i: number) => (
