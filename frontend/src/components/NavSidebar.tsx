      ...
      >
        ...
        <div className="flex items-center justify-between px-5 h-14">
          ...
          <div className="hidden md:flex items-center gap-1">
            {['/', '/analyze', '/diff', '/graph'].map((path, i) => {
              const labels = ['Поиск', 'Анализ', 'Сравнение', 'Граф'];
              const targetPath = (path === '/analyze' && selectedDocId) ? `/analyze/${selectedDocId}` : path;
              ...
              return (
                ...
                >
                  {labels[i]}
                  {i === 1 && activeScope.length > 0 && (
                    <span className="w-4 h-4 bg-indigo-500 text-[10px] flex items-center justify-center rounded-full text-white shadow-lg animate-pulse">
                      {activeScope.length}
                    </span>
                  )}
                </button>
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  {card.links.map((lnk, i) => {
                    const targetHref = (lnk.href === '/analyze' && selectedDocId) ? `/analyze/${selectedDocId}` : lnk.href;
                    return (
                      <button
                        key={i}
                        onClick={() => go(targetHref)}
