...
export const analyzeDocument = async (docId: string, docIds?: string[]) => {
  const params = docIds && docIds.length > 0 ? `?doc_ids=${docIds.join('&doc_ids=')}` : '';
  return (await api.get(`/analyze/${docId}${params}`)).data;
};
...
export const runGlobalAudit = async (docIds?: string[]) => {
...
export const getHeatmapData = async (docIds?: string[]) => {
  ...
  return (await api.get(`/graph/heatmap${params}`)).data;
};

export const getGraphHtmlUrl = (filterType = 'Все', docIds?: string[]) => {
  const scopeParam = docIds && docIds.length > 0 ? `&doc_ids=${docIds.join(',')}` : '';
  return `http://127.0.0.1:8000/api/v1/graph/html?filter_type=${encodeURIComponent(filterType)}${scopeParam}`;
};
