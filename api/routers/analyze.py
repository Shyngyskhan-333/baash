...
@router.get("/api/v1/analyze/{doc_id}", response_model=AnalyzeResponse)
async def analyze_document_id(doc_id: str):
    return await _process_analyze(doc_id)

@router.get("/api/v1/analyze/by-url", response_model=AnalyzeResponse)
async def analyze_document_url(url: str = Query(..., description="Adilet URL")):
    try:
...
async def _process_analyze(doc_id: str) -> AnalyzeResponse:
    ...
    # 2. Extract issues
    analysis_data = nlp_service.analyze_document(doc_id)
