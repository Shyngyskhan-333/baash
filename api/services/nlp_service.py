...
class NLPService:
    ...
    def analyze_document_fast(self, doc_id: str, doc_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Ensure the document exists in the graph node list (collective ecosystem entry)
        ...
        # If doc_ids scope is provided, search deeper (top 50) to find relevant matches.
        ...
        for i, (chunk, row_scores, row_indices) in enumerate(zip(doc_chunks, scores, indices)):
            for score, j in zip(row_scores, row_indices):
                ...
                # If global scope is active, only compare with documents in that scope
